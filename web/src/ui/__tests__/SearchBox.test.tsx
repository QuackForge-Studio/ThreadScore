...
describe('SearchBox', () => {
  ...
  it('shows request button for unknown threads URL', async () => {
    renderBox();
    fireEvent.change(screen.getByPlaceholderText(/Tìm kiếm hoặc dán link Threads/i), {
  ...
  it('submits search on Enter key', async () => {
    renderBox();
    const input = screen.getByPlaceholderText(/Tìm kiếm hoặc dán link Threads/i);
  ...
  it('shows English strings when language is English', async () => {
    renderBox('en');
    fireEvent.change(screen.getByPlaceholderText(/Search or paste a Threads link/i), {
      target: { value: 'https://www.threads.net/@x/post/C1' },
    });
